...
const EditProfileForm = forwardRef(function EditProfileForm(
  ...
  // ── Form draft state ─────────────────────────────────────────────────────
  const [bioText, setBioText] = useState(myProfile?.bio || "");
  ...
  const [draftInterestedIn, setDraftInterestedIn] = useState([]);
  ...
  const [draftShowMyUniversity, setDraftShowMyUniversity] = useState(true);
  const [draftShowMeOnApp, setDraftShowMeOnApp] = useState(true);
  const [draftShowAge, setDraftShowAge] = useState(true);
  ...
  useEffect(() => {
    ...
    setDraftInterestedIn(
      matchMulti(interestedInOptions, myProfile?.interestedIn),
    );
    setDraftLanguages(matchMulti(languageOptions, myProfile?.spokenLanguages));
    setDraftPets(matchMulti(petOptions, myProfile?.pets));
    ...
    setDraftShowMyUniversity(myProfile?.showMyUniversity !== false);
    setDraftShowMeOnApp(myProfile?.showMeOnApp !== false);
    setDraftShowAge(myProfile?.showAge !== false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [myProfile]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      const matched = myProfile.interestedIn
        ...
        .filter(Boolean);
      if (matched.length > 0) setDraftInterestedIn(matched);
    }
    ...
    if (
      ...
      const matched = myProfile.spokenLanguages
        ...
        .filter(Boolean);
      if (matched.length > 0) setDraftLanguages(matched);
    }
    ...
    if (
      ...
      const matched = myProfile.pets
        ...
        .filter(Boolean);
      if (matched.length > 0) setDraftPets(matched);
    }
  }, [
    myProfile,
    cityOptions,
  ...
  const toggleInterestedIn = useCallback((opt) => {
    if (!opt) return;
    setDraftInterestedIn((prev) =>
      prev.some((p) => p?.id === opt.id)
  ...
  const applyCity = useCallback(
    (opt) => {
      if (!opt || opt.id === draftCity?.id) return;
      setDraftCity(opt);
      setDraftDistrict(null);
      setDistrictOptions([]);
      setDistrictsLoading(true);
  ...
  const onLanguageConfirm = useCallback(
    (enumNames) => {
      ...
      setDraftLanguages(languageOptions.filter((o) => set.has(o.enumName)));
    },
    [languageOptions],
  );
        ...
        {[
          {
            ...
            set: setDraftShowMyUniversity,
          },
          {
            key: "app",
            label: "Beni uygulamada göster",
            value: draftShowMeOnApp,
            set: setDraftShowMeOnApp,
          },
          {
            key: "age",
            label: "Yaşımı göster",
