...
/**
 * Mağaza şeridinin sol kartı: Fire paketi satın alma girişi + bakiye.
 * Kabuk ve bakiye çözümü paylaşılıyor (ConsumableShopCard /
 * consumableCardBalance) — burada yalnız ürüne özgü kısım var.
 *
 * Kart premium'da da görünür: Fire'ın "sınırsız" hâli yok, premium'da bile
...
export default function FireCard({
  ...
  const subtitle =
    balance.kind === "unknown"
      ? t("profile.fireCard.subtitleUnknown")
      : balance.remaining > 0
        ? t("profile.fireCard.subtitleCount", { count: balance.remaining })
        : t("profile.fireCard.subtitleEmpty");
  ...
  return (
    <>
      <ConsumableShopCard
        testID="fire-card"
        cardWidth={cardWidth}
        title={t("profile.fireCard.title")}
        subtitle={subtitle}
